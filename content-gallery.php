	...
	<div class="entry-content">
		<?php if ( post_password_required() ) : ?>
			...
			<?php else : ?>
				...
				<p><em><?php printf( _n( 'This gallery contains <a %1$s>%2$s photo</a>.', 'This gallery contains <a %1$s>%2$s photos</a>.', $total_images, 'seismicnyc' ),
						'href="' . get_permalink() . '" title="' . sprintf( esc_attr__( 'Permalink to %s', 'seismicnyc' ), the_title_attribute( 'echo=0' ) ) . '" rel="bookmark"',
	...
	<footer class="entry-meta">
		<?php if ( 'post' == get_post_type() ) : // Hide category and tag text for pages on Search ?>
			<?php
				...
				$categories_list = get_the_category_list( __( ', ', 'seismicnyc' ) );
				if ( $categories_list && seismicnyc_categorized_blog() ) :
			?>
			<span class="cat-links">
				<?php printf( __( 'Posted in %1$s', 'seismicnyc' ), $categories_list ); ?>
			</span>
			<?php endif; // End if categories ?>

			<?php
				/* translators: used between list items, there is a space after the comma */
				$tags_list = get_the_tag_list( '', __( ', ', 'seismicnyc' ) );
				if ( $tags_list ) :
			?>
			<span class="tag-links">
				<span class="sep"> | </span>
				<?php printf( __( 'Tagged %1$s', 'seismicnyc' ), $tags_list ); ?>
			</span>
